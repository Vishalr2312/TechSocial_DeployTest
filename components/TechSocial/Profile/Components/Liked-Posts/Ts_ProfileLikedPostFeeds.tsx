...
const Ts_ProfileLikedPostFeeds = ({ clss = "", reaction = "" }) => {
  ...
  const fetchAllPosts = useCallback(async () => {
    try {
      ...
      const firstRes = await axiosCall<ApiResponse>({
        ENDPOINT: "posts/view-liked-posts?page=1",
        METHOD: "GET",
      });
      ...
      const otherPageRequests = Array.from(
        ...
        (_, i) =>
          axiosCall<ApiResponse>({
            ENDPOINT: `posts/view-liked-posts?page=${i + 2}`,
            METHOD: "GET",
          }).catch(() => null)
      );
...
export default Ts_ProfileLikedPostFeeds;
